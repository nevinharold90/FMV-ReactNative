...
const PastDeliveries: React.FC<PastDeliveriesProps> = ({ navigation }) => {
  ...
  const [deliveries, setDeliveries] = useState<any[]>([]); // State to store all deliveries
    ...
    <SafeAreaView className="flex-1 bg-white">
      ...
      {deliveries.length > 0 ? (
        <FlatList
          ...
          data={deliveries}
          keyExtractor={(item) =>
            item.delivery_id ? item.delivery_id.toString() : `${item.purchase_order_id}-${item.delivery_no}`
          }
          renderItem={({ item }) => (
            <View
              className={`mb-4 p-5 bg-gray-200 rounded-lg shadow-md border ${
                item.has_damages ? 'border-red-500' : 'border-blue-500'
              }`}
            >
              <View className="w-full flex flex-row items-center px-1">
                <Text
                  className={`font-bold text-xl ${
                    item.has_damages ? 'text-red-500' : 'text-blue-600'
                  }`}
                >
                  POID No:
                </Text>
              ...
              <View
                className={`p-4 rounded-2xl ${item.has_damages ? 'bg-red-500' : 'bg-blue-500'}`}
              >
                <Text className="text-white font-bold">Note:</Text>
                <Text className="mt-2 text-lg font-bold text-white">
                  {item.has_damages
                    ? '"Successful with Damage/Loss of Products Refunded"'
                    : '"Successful Delivery without Damage/Loss of products"'}
                </Text>
              </View>
            </View>
          )}
